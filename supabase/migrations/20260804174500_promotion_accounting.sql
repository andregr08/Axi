-- ============================================================
-- AXI: CONTABILIDAD AUTOMÁTICA DE PROMOCIONES
-- ============================================================

-- Congela en el pago la información promocional vigente
-- al momento de crear la transacción.

alter table public.payment_transactions
  add column if not exists gross_trip_amount numeric(12,2)
    not null default 0,
  ...
  add column if not exists promotion_code_id uuid null,

  add column if not exists promotion_redemption_id uuid null,

  add column if not exists net_trip_amount numeric(12,2)
    not null default 0;
...
alter table public.payment_transactions
  drop constraint if exists
    payment_transactions_gross_trip_amount_check;

alter table public.payment_transactions
  add constraint
    payment_transactions_gross_trip_amount_check
  check (gross_trip_amount >= 0);


alter table public.payment_transactions
  drop constraint if exists
    payment_transactions_promotion_discount_check;
...
alter table public.payment_transactions
  drop constraint if exists
    payment_transactions_net_trip_amount_check;

alter table public.payment_transactions
  add constraint
    payment_transactions_net_trip_amount_check
  check (net_trip_amount >= 0);


alter table public.payment_transactions
  drop constraint if exists
    payment_transactions_promotion_amounts_check;

alter table public.payment_transactions
  add constraint
    payment_transactions_promotion_amounts_check
  check (
    promotion_discount <= gross_trip_amount
    and net_trip_amount =
      round(
        gross_trip_amount - promotion_discount,
        2
      )
  );
...
begin
  if not exists (
    ...
    from pg_constraint
    where conname =
      'payment_transactions_promotion_code_id_fkey'
      and conrelid =
...
begin
  if not exists (
    ...
    from pg_constraint
    where conname =
      'payment_transactions_promotion_redemption_id_fkey'
      and conrelid =
        'public.payment_transactions'::regclass
  ) then
    alter table public.payment_transactions
      add constraint
        payment_transactions_promotion_redemption_id_fkey
      foreign key (promotion_redemption_id)
...
create index if not exists
  payment_transactions_promotion_code_id_idx
...
create index if not exists
  payment_transactions_promotion_redemption_id_idx
on public.payment_transactions (
...
create index if not exists
  payment_transactions_promotion_code_idx
on public.payment_transactions (
  promotion_code
)
...
-- ============================================================
-- CREATE_TRIP_PAYMENT CON PROMOCIÓN CONGELADA
-- ============================================================
...
begin
  ...
  into v_promotion_redemption_id
...
-- ============================================================
-- PÓLIZA DE VIAJE CON GASTO PROMOCIONAL
-- ============================================================
...
declare
  ...
  v_promotions_expense_account uuid;
  ...
  v_promotion_discount numeric(18,2);
...
begin
  ...
  */
  ...
  select id into v_promotions_expense_account
  from public.financial_accounts
  where code = 'expense.promotions'
    and status = 'active';
  ...
  if v_platform_income_account is null
     or v_promotions_expense_account is null
  ...
  v_promotion_discount :=
    round(
      greatest(
        coalesce(v_payment.promotion_discount, 0),
        0
      ),
      2
    );

  if v_promotion_discount >
     round(coalesce(v_payment.gross_trip_amount, 0), 2) then
    raise exception
      'El descuento promocional del pago % supera el monto bruto',
      p_payment_id;
  end if;
    ...
    v_entries :=
      v_entries || jsonb_build_array(
        jsonb_build_object(
          ...
          'description',
            'Costo de promoción financiada por AXI',
          ...
          'metadata',
            jsonb_build_object(
              'promotion_code',
                v_payment.promotion_code,

              'promotion_code_id',
                v_payment.promotion_code_id,

              'promotion_redemption_id',
                v_payment.promotion_redemption_id,

              'gross_trip_amount',
                v_payment.gross_trip_amount,

              'net_trip_amount',
                v_payment.net_trip_amount
            )
        )
      );
  end if;
